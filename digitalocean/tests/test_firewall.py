...
class TestFirewall(BaseTest):
    ...
    def test_load(self):
        ...
        self.assertEqual(f.inbound_rules[0].sources[0].load_balancer_uids,
                         ["12345"])
        self.assertEqual(f.inbound_rules[0].sources[0].addresses, [])
        self.assertEqual(f.inbound_rules[0].sources[0].tags, [])
        self.assertEqual(f.outbound_rules[0].ports, "80")
        self.assertEqual(f.outbound_rules[0].protocol, "tcp")
        self.assertEqual(
            f.outbound_rules[0].destinations[0].load_balancer_uids, [])
        self.assertEqual(f.outbound_rules[0].destinations[0].addresses,
                         ["0.0.0.0/0", "::/0"])
        self.assertEqual(f.outbound_rules[0].destinations[0].tags, [])
